...
function Recipients() {
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 md:gap-6 gap-4">
        {recipientsData?.map((contact) => (
          <RecipientCard key={contact.id} contact={contact} />
        ))}
        <AddRecipientModal
          trigger={
            <Card className="flex flex-col items-center justify-center border-gray-100 border! rounded-2xl! bg-white md:p-6 p-4 shadow-[0_0_4px_0_rgba(0,0,0,0.16)]  transition-all h-full min-h-70 cursor-pointer hover:bg-gray-50/50 group">
